...
import { AlertTriangle, CheckCircle2, Eraser, Eye, Loader2, Moon, RefreshCw, Send, Layers, Zap } from "lucide-react";
...
export default function CloudModulePage() {
  ...
  const doPush = useCallback(async () => {
    ...
    try {
      const r = await fetch("/api/cloud-module/push", {
        ...
        body: JSON.stringify({ persona: selected, sleep_after: sleepAfter, clear_before: clearBefore }),
      });
      const d = await r.json();
      appendLog(
        d.ok
          ? `push ${selected} ok, bw=${d.bw_len} bytes, queued for next device connect` +
            (d.clear_before ? " (将先全白清屏)" : "") +
  ...
  }, [selected, refreshStatus, sleepAfter, clearBefore, appendLog]);
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <label className="flex items-center gap-2 text-xs text-ink-light cursor-pointer select-none">
              ...
              />
              <Eraser size={12} className="text-ink-light" />
              <span>先全屏清白（推图前先发一帧全白数据，消除顶栏残影，耗时多 ~5s）</span>
            </label>
            <label className="flex items-center gap-2 text-xs text-ink-light cursor-pointer select-none">
              <input
                type="checkbox"
